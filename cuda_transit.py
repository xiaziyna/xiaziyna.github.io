...
# Scatter parallelism (each task takes an element of the noise covariance and 'scatters' it into transit detection tests

#=============================================
# Comparison CPU kernel (runs on a single core)
# Scatter pattern: iterates over elements of covariance
def CPU_transit_num(output_array, input_array, half_N):
    for i in range(len(input_array)):
        for p in range(1, half_N+1):
            output_array[p-1,i%p] += input_array[i]

def CPU_transit_den(output_array, input_array, half_N):
...
N_full = 8*720 # no. of samples in lightcurve: here modelling 8-days of 2-minute short cadence data
...
# CUDA kernel "parallel_num" takes y_d as input and outputs the numerator of every detection test, defined over P in [1.. N/2], epoch in [1.. N/2]

start.record()
start.synchronize()

parallel_transit_num = mod.get_function("parallel_transit_num")
parallel_transit_num(drv.Out(output_num), drv.In(y_d), np.int32(half_N), block=(blockwidth,blockwidth,1), grid=(gridwidth, int(gridwidth/2), 1))

end.record()
end.synchronize()
secs = start.time_till(end)*1e-3
print ('(GPU) Time to calculate numerator of tests: ', secs)
#=============================================
...
parallel_transit_den = mod.get_function("parallel_transit_den")
parallel_transit_den(drv.Out(output_den), drv.In(K_d), np.int32(half_N), block=(blockwidth, blockwidth, 1), grid=(gridwidth,gridwidth))

end.record()
end.synchronize()

secs = start.time_till(end)*1e-3
print ('(GPU) Time to calculate denominator of tests: ', secs)
...
CPU_transit_num(output_num, y_d, half_N)
...
CPU_transit_den(output_den, K_d, half_N)
